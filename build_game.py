...
function initLevel(cols,rows){{ levelWidth=cols*TILE; mapTiles=[]; for(var r=0;r<rows;r++){{ mapTiles[r]=[]; for(var c=0;c<cols;c++) mapTiles[r][c]=0; }} for(var c=0;c<cols;c++){{ mapTiles[rows-1][c]=1; mapTiles[rows-2][c]=1; }}     furniture=[]; collectibles=[]; enemies=[]; lightningBolts=[]; sneezeClouds=[]; bossSpecialTimer=0; }}
...
function drawLevelComplete(){{
    ...
    if(levelCompleteTimer<120){{
        ...
        X.fillStyle=hover?'#00ccff':'#0088cc'; X.shadowColor='#00ccff'; X.shadowBlur=hover?15:0;
        ...
        X.font='bold 16px Courier New'; X.fillStyle='#fff'; X.fillText('CONTINUAR',W/2,btnY+27);
    }}
}}

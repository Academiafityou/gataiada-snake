...
var LEVELS = [
    ...
    {name:'Planeta dos Sachês',desc:'Fase bonus - Colete todos os sachês!',theme:'inverted',color:'#FF69B4',bg1:'#1a0a2e',bg2:'#3a1a4e',ground:'#D63384',platform:'#FF85C0',enemies:3,boss_hp:5,boss_name:'Thia do Espirro'},
    {name:'Castelo da Princesa Dorothy',desc:'Jardim encantado - Desvie dos raios!',theme:'castle',color:'#FF1493',bg1:'#1a0a2e',bg2:'#2a1a4e',ground:'#FF69B4',platform:'#FFB6C1',enemies:10,boss_hp:7,boss_name:'Princesa Dóx'},
    {name:'Espaço Kids!',desc:'Festa infantil gigante - Derrote a Xepa Grande!',theme:'kids',color:'#FF1493',bg1:'#FF69B4',bg2:'#FFD700',ground:'#FF1493',platform:'#FF69B4',enemies:12,boss_hp:10,boss_name:'Xepa Grande'}
];
...
var pitoFaraImg=new Image(); pitoFaraImg.src='{boss_urls["pito_fara"]}';
var thiaImg=new Image(); thiaImg.src='{boss_urls["thia_espirro"]}';
var princesaDoxImg=new Image(); princesaDoxImg.src='{boss_urls["princesa_dox"]}';
...
var lightningBolts=[], sneezeClouds=[], bossSpecialTimer=0;
var kidsBalloons=[], kidsConfetti=[], kidsBalls=[], kidsToys=[];
...
function buildHouseLevel() {{
    ...
    cameraX=0; projectiles=[]; particles=[]; lightningBolts=[]; sneezeClouds=[]; bossSpecialTimer=0; game.time=180;
}}

function initLevel(cols,rows){{ levelWidth=cols*TILE; mapTiles=[]; for(var r=0;r<rows;r++){{ mapTiles[r]=[]; for(var c=0;c<cols;c++) mapTiles[r][c]=0; }} for(var c=0;c<cols;c++){{ mapTiles[rows-1][c]=1; mapTiles[rows-2][c]=1; }}     furniture=[]; collectibles=[]; enemies=[]; lightningBolts=[]; sneezeClouds=[]; bossSpecialTimer=0; kidsBalloons=[]; kidsConfetti=[]; kidsBalls=[]; kidsToys=[]; }}
...
function buildLevel5(){{
    var L=LEVELS[4], rows=TILE_ROWS, cols=80;
    initLevel(cols,rows);
    addPlat(6,rows-5,8); addPlat(16,rows-7,6); addPlat(24,rows-4,7);
    addPlat(33,rows-6,6); addPlat(41,rows-8,5);
    addPlat(48,rows-5,7); addPlat(56,rows-7,5);
    addPlat(14,rows-11,4); addPlat(38,rows-12,3); addPlat(52,rows-10,4);
    for(var c=62;c<cols-5;c++) mapTiles[rows-2][c]=2;
    addCoins(8*TILE,(rows-6)*TILE,4,35); addCoins(18*TILE,(rows-8)*TILE,3,30);
    addCoins(26*TILE,(rows-5)*TILE,4,35); addCoins(35*TILE,(rows-7)*TILE,3,30);
    addCoins(43*TILE,(rows-9)*TILE,3,30); addCoins(50*TILE,(rows-6)*TILE,4,35);
    addCoins(58*TILE,(rows-8)*TILE,3,30);
    addSache(8*TILE,(rows-6)*TILE-20); addSache(16*TILE,(rows-8)*TILE-20);
    addSache(24*TILE,(rows-5)*TILE-20); addSache(33*TILE,(rows-7)*TILE-20);
    addSache(41*TILE,(rows-9)*TILE-20); addSache(48*TILE,(rows-6)*TILE-20);
    addSache(56*TILE,(rows-8)*TILE-20); addSache(14*TILE,(rows-12)*TILE-20);
    addSache(38*TILE,(rows-13)*TILE-20); addSache(52*TILE,(rows-11)*TILE-20);
    addSache(64*TILE,(rows-6)*TILE-20); addSache(70*TILE,(rows-5)*TILE-20);
    addHeart(14*TILE,(rows-12)*TILE); addHeart(52*TILE,(rows-11)*TILE);
    addExtraLife(38*TILE,(rows-13)*TILE-10);
    enemies.push({{x:10*TILE,y:(rows-6)*TILE-24,w:20,h:20,vx:1,vy:0,hp:1,type:'fly',alive:true,grounded:false,flying:true}});
    enemies.push({{x:30*TILE,y:(rows-7)*TILE-24,w:20,h:20,vx:-1.2,vy:0,hp:1,type:'fly',alive:true,grounded:false,flying:true}});
    enemies.push({{x:50*TILE,y:(rows-8)*TILE-24,w:20,h:20,vx:1,vy:0,hp:1,type:'fly',alive:true,grounded:false,flying:true}});
...
function buildLevel6(){{
    ...
    initLevel(cols,rows);
    for(var c=10;c<25;c++){{ mapTiles[rows-2][c]=5; }}
    ...
    furniture.push({{x:12*TILE,y:(rows-5)*TILE,w:3*TILE,h:3*TILE,type:'flowerbed',color:'#FF69B4'}});
    furniture.push({{x:30*TILE,y:(rows-7)*TILE,w:2*TILE,h:2*TILE,type:'fountain',color:'#87CEEB'}});
    furniture.push({{x:60*TILE,y:(rows-8)*TILE,w:3*TILE,h:2*TILE,type:'flowerbed',color:'#FFB6C1'}});
    furniture.push({{x:86*TILE,y:(rows-9)*TILE,w:2*TILE,h:2*TILE,type:'flowerbed',color:'#FF1493'}});
    furniture.push({{x:105*TILE,y:(rows-8)*TILE,w:3*TILE,h:2*TILE,type:'flowerbed',color:'#DB7093'}});
...
function buildLevel7(){{
    ...
    initLevel(cols,rows);
    for(var c=130;c<cols-5;c++){{ mapTiles[rows-2][c]=2; mapTiles[rows-3][c]=2; }}
    addPlat(8,rows-5,6); addPlat(16,rows-7,5); addPlat(24,rows-4,7);
    addPlat(14,rows-10,4); addPlat(33,rows-8,5);
    addPlat(40,rows-5,8); addPlat(50,rows-7,4); addPlat(56,rows-10,6);
    addPlat(64,rows-5,5); addPlat(70,rows-8,7); addPlat(78,rows-6,4);
    ...
    addPlat(110,rows-12,3); addPlat(125,rows-10,4);
    addPlat(125,rows-5,8); addPlat(135,rows-7,5);
    for(var i=0;i<40;i++) kidsToys.push({{x:Math.random()*cols*TILE,y:(rows-3)*TILE-Math.random()*10,type:['block','ball','duck','star','ring'][Math.floor(Math.random()*5)],color:['#FF0000','#FF6600','#FFCC00','#00CC00','#0066FF','#CC00FF','#FF69B4','#00CED1'][Math.floor(Math.random()*8)],size:8+Math.random()*12,angle:Math.random()*Math.PI*2}});
    for(var i=0;i<15;i++) kidsBalloons.push({{x:Math.random()*cols*TILE,y:Math.random()*H*0.7,vx:(Math.random()-0.5)*0.3,vy:-0.3-Math.random()*0.3,color:['#FF0000','#FF6600','#FFCC00','#00CC00','#0066FF','#CC00FF','#FF69B4','#FF1493','#00CED1','#FFD700'][Math.floor(Math.random()*10)],r:12+Math.random()*10,phase:Math.random()*Math.PI*2}});
    for(var i=0;i<150;i++) kidsConfetti.push({{x:Math.random()*cols*TILE,y:-Math.random()*H*2,vx:(Math.random()-0.5)*1.5,vy:1+Math.random()*2,rot:Math.random()*Math.PI*2,rotSpeed:(Math.random()-0.5)*0.2,color:['#FF0000','#FF6600','#FFCC00','#00CC00','#0066FF','#CC00FF','#FF69B4','#FFD700'][Math.floor(Math.random()*8)],size:3+Math.random()*5,life:9999}});
    for(var i=0;i<8;i++) kidsBalls.push({{x:10*TILE+i*15*TILE+Math.random()*5*TILE,y:(rows-3)*TILE-14,r:10+Math.random()*6,color:['#FF0000','#FF6600','#FFCC00','#00CC00','#0066FF','#CC00FF','#FF69B4','#FFD700'][i%8],vx:(Math.random()>0.5?1:-1)*(0.8+Math.random()*1.2),stripe:Math.random()>0.5}});
    addCoins(10*TILE,(rows-6)*TILE,4,35); addCoins(18*TILE,(rows-8)*TILE,3,30);
    addCoins(35*TILE,(rows-9)*TILE,4,30); addCoins(42*TILE,(rows-6)*TILE,5,35);
    addCoins(52*TILE,(rows-8)*TILE,3,30); addCoins(58*TILE,(rows-11)*TILE,4,25);
    addCoins(66*TILE,(rows-6)*TILE,3,35); addCoins(72*TILE,(rows-9)*TILE,4,30);
    ...
    addExtraLife(94*TILE,(rows-8)*TILE-10);
    var gy=(rows-3)*TILE-24;
    enemies.push({{x:12*TILE,y:(rows-6)*TILE-24,w:24,h:30,vx:1.2,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:26*TILE,y:gy,w:24,h:30,vx:1.0,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:36*TILE,y:(rows-9)*TILE-24,w:24,h:30,vx:1.3,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:42*TILE,y:gy,w:24,h:30,vx:1.1,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:52*TILE,y:(rows-8)*TILE-24,w:24,h:30,vx:1.4,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:66*TILE,y:gy,w:24,h:30,vx:1.0,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:74*TILE,y:(rows-9)*TILE-24,w:24,h:30,vx:1.2,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:86*TILE,y:gy,w:24,h:30,vx:1.3,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:102*TILE,y:(rows-10)*TILE-24,w:24,h:30,vx:1.1,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:112*TILE,y:gy,w:24,h:30,vx:1.4,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:120*TILE,y:(rows-8)*TILE-24,w:24,h:30,vx:1.2,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    enemies.push({{x:130*TILE,y:gy,w:24,h:30,vx:1.0,vy:0,hp:2,type:'xepa',alive:true,grounded:false}});
    boss={{x:(cols-8)*TILE,y:(rows-10)*TILE,w:160,h:180,hp:L.boss_hp,maxHp:L.boss_hp,vx:-0.8,vy:0,alive:true,phase:0,phaseTimer:0,attackTimer:0,color:'#FF1493',name:'Xepa Grande',big:true,grounded:false}};
    spawnPlayer(rows,3);
}}
...
function updatePlayer(){{
    ...
    var colL2x=Math.floor(newX/TILE), colR2x=Math.floor((newX+player.w)/TILE);
    if(getTile(colL2,rowB2)>0||getTile(colR2,rowB2)>0||getTile(colL2x,rowB2)>0||getTile(colR2x,rowB2)>0){{ player.grounded=true; if(getCatPower()!=='Triple Jump') player.jumpCount=0; }}
    ...
    var blockedY=false;
    var colLv=Math.floor(player.x/TILE), colRv=Math.floor((player.x+player.w)/TILE);
    var rowTN=Math.floor(newY/TILE), rowBN=Math.floor((newY+player.h)/TILE);
    if(player.vy>=0){{
        for(var r=rowBN;r>=rowTN;r--) for(var c=colLv;c<=colRv;c++){{ var t=getTile(c,r); if(t===1||t===2||t===5||t===7){{ player.y=r*TILE-player.h; player.grounded=true; player.jumpCount=0; blockedY=true; break; }} if(blockedY) break; }}
    }} else {{
        for(var r=rowTN;r<=rowBN;r++) for(var c=colLv;c<=colRv;c++){{ var t=getTile(c,r); if(t===1||t===2||t===5||t===7){{ var pushY=(r+1)*TILE; var hr=Math.floor(pushY/TILE); if(getTile(c,hr)>0){{ for(var rr=r+1;rr<TILE_ROWS;rr++){{ if(getTile(c,rr)===0){{ pushY=rr*TILE; break; }} }} }} player.y=pushY; blockedY=true; break; }} if(blockedY) break; }}
    }}
...
function updateBoss(){{
    ...
    if(Math.abs(dx)<600){{
        ...
        bossSpecialTimer++;
        if(game.currentLevel===4&&bossSpecialTimer%120===0){{
            for(var si=0;si<8;si++){{ var ang=si*Math.PI/4; sneezeClouds.push({{x:boss.x+boss.w/2,y:boss.y+boss.h/2,vx:Math.cos(ang)*3,vy:Math.sin(ang)*3,life:60,r:25}}); }}
            spawnParticles(boss.x+boss.w/2,boss.y+boss.h/2,'#fff',12);
        }}
        if(game.currentLevel===5&&bossSpecialTimer%90===0){{
            var lx=boss.x+boss.w/2+(Math.random()-0.5)*400;
            lightningBolts.push({{x:lx,y:0,h:H-80,life:25,w:6,flash:15}});
            for(var li=0;li<5;li++){{ spawnParticles(lx,H-80-TILE,'#ffff00',3); }}
        }}
        if(game.currentLevel===6&&bossSpecialTimer%80===0){{
            for(var si=0;si<10;si++){{ var ang=si*Math.PI/5; var spd=3+Math.random()*2;
                projectiles.push({{x:boss.x+boss.w/2,y:boss.y+boss.h/2,vx:Math.cos(ang)*spd,vy:Math.sin(ang)*spd-2,w:10,h:10,friendly:false,life:80,color:['#FF0000','#FFCC00','#00CC00','#0066FF','#CC00FF'][si%5]}});
            }}
            spawnParticles(boss.x+boss.w/2,boss.y+boss.h/2,'#FFD700',15);
        }}
    }}
    for(var i=sneezeClouds.length-1;i>=0;i--){{ var sc=sneezeClouds[i]; sc.x+=sc.vx; sc.y+=sc.vy; sc.life--; sc.r+=0.3; if(sc.life<=0){{ sneezeClouds.splice(i,1); continue; }} if(rectOverlap(player,{{x:sc.x-sc.r/2,y:sc.y-sc.r/2,w:sc.r,h:sc.r}})&&player.invincible<=0){{ playerHit(1); }} }}
    for(var i=lightningBolts.length-1;i>=0;i--){{ var lb=lightningBolts[i]; lb.life--; lb.flash--; if(lb.life<=0){{ lightningBolts.splice(i,1); continue; }} if(lb.flash<=0&&Math.abs(player.x+player.w/2-lb.x)<lb.w+player.w/2){{ playerHit(1); lightningBolts.splice(i,1); }} }}
}}
...
function updateProjectiles(){{
    for(var i=projectiles.length-1;i>=0;i--){{
        ...
        if(p.friendly&&boss&&boss.alive&&rectOverlap(p,boss)){{
            ...
            if(boss.hp<=0){{ boss.alive=false; game.score+=2000; game.defeatedBossName=boss.name||'BOSS'; spawnParticles(boss.x+boss.w/2,boss.y+boss.h/2,'#ff00ff',40); for(var ci=0;ci<30;ci++){{ spawnParticles(boss.x+Math.random()*boss.w,boss.y+Math.random()*boss.h,['#ff00ff','#ffcc00','#00ffcc','#ff6600','#ffff00'][Math.floor(Math.random()*5)],6); }} game.state='levelcomplete'; levelCompleteTimer=180; game.levelsUnlocked[Math.min(game.currentLevel+1,6)]=true; }}
        }}
    }}
}}
...
function drawFurniture(){{
    ...
    for(var i=0;i<furniture.length;i++){{
        ...
        switch(f.type){{
            ...
            case 'flowerbed': X.fillStyle='#5a3a1a'; X.fillRect(f.x,f.y,f.w,f.h); X.fillStyle='#228B22'; X.fillRect(f.x+2,f.y+2,f.w-4,f.h/2); var fcolors=['#FF69B4','#FF1493','#FFB6C0','#FF00FF','#DB7093']; for(var fi=0;fi<Math.floor(f.w/20);fi++){{ X.fillStyle=fcolors[fi%fcolors.length]; X.beginPath(); X.arc(f.x+12+fi*20,f.y+f.h/2-2,6,0,Math.PI*2); X.fill(); X.fillStyle='#FFD700'; X.beginPath(); X.arc(f.x+12+fi*20,f.y+f.h/2-2,2,0,Math.PI*2); X.fill(); }} break;
            case 'fountain': X.fillStyle='#B0C4DE'; X.fillRect(f.x,f.y,f.w,f.h); X.fillStyle='#87CEEB'; X.fillRect(f.x+4,f.y+4,f.w-8,f.h-8); X.fillStyle='rgba(135,206,235,0.6)'; X.beginPath(); X.arc(f.x+f.w/2,f.y+f.h/2,f.w/3,0,Math.PI*2); X.fill(); X.fillStyle='#4682B4'; X.fillRect(f.x+f.w/2-2,f.y-10,4,10); var wf=Date.now()*0.005; X.fillStyle='rgba(135,206,235,0.5)'; for(var wi=0;wi<3;wi++){{ X.beginPath(); X.arc(f.x+f.w/2+Math.sin(wf+wi)*8,f.y-8-Math.abs(Math.cos(wf+wi))*6,3,0,Math.PI*2); X.fill(); }} break;
        }}
    }}
    X.restore();
}}
...
function drawDesertBG(){{
    var grad=X.createLinearGradient(0,0,0,H);
    grad.addColorStop(0,'#1a0a2e'); grad.addColorStop(0.3,'#2e1a0a'); grad.addColorStop(0.5,'#D4A017'); grad.addColorStop(1,'#C8A84E');
    X.fillStyle=grad; X.fillRect(0,0,W,H);
    X.fillStyle='rgba(255,200,50,0.08)';
    for(var i=0;i<3;i++){{ X.beginPath(); X.arc(W*0.7+i*80,80,60+i*30,0,Math.PI*2); X.fill(); }}
    var px1=100-cameraX*0.05, px2=500-cameraX*0.08, px3=850-cameraX*0.03;
    X.fillStyle='#8B7355'; X.beginPath(); X.moveTo(px1,280); X.lineTo(px1+120,100); X.lineTo(px1+240,280); X.closePath(); X.fill();
    X.fillStyle='#7A6545'; X.beginPath(); X.moveTo(px1+10,280); X.lineTo(px1+120,108); X.lineTo(px1+120,280); X.closePath(); X.fill();
    X.fillStyle='#9B8365'; X.beginPath(); X.moveTo(px2,280); X.lineTo(px2+180,60); X.lineTo(px2+360,280); X.closePath(); X.fill();
    X.fillStyle='#8A7255'; X.beginPath(); X.moveTo(px2+15,280); X.lineTo(px2+180,68); X.lineTo(px2+180,280); X.closePath(); X.fill();
    X.fillStyle='#A09070'; X.beginPath(); X.moveTo(px3,280); X.lineTo(px3+90,140); X.lineTo(px3+180,280); X.closePath(); X.fill();
    X.fillStyle='#C8A84E'; X.fillRect(0,280,W,2);
    X.strokeStyle='rgba(218,165,32,0.3)'; X.lineWidth=1;
    for(var l=0;l<5;l++){{ X.beginPath(); X.moveTo(0,282+l*8); X.lineTo(W,282+l*8); X.stroke(); }}
    X.fillStyle='#DAA520'; var obX=300-cameraX*0.12;
    X.fillRect(obX,240,8,40); X.fillRect(obX+60,230,8,50); X.fillRect(obX+120,245,8,35);
    X.beginPath(); X.moveTo(obX+4,240); X.lineTo(obX-4,234); X.lineTo(obX+12,234); X.closePath(); X.fill();
    X.beginPath(); X.moveTo(obX+64,230); X.lineTo(obX+56,222); X.lineTo(obX+72,222); X.closePath(); X.fill();
    X.beginPath(); X.moveTo(obX+124,245); X.lineTo(obX+116,238); X.lineTo(obX+132,238); X.closePath(); X.fill();
}}

function drawGardenBG(){{
    var L=LEVELS[5];
    var nearBoss=boss&&boss.alive&&Math.abs(player.x-boss.x)<W;
    var skyTop=nearBoss?'#0a0015':'#87CEEB', skyBot=nearBoss?'#1a0a2e':'#E0F0FF';
    var grad=X.createLinearGradient(0,0,0,H);
    grad.addColorStop(0,skyTop); grad.addColorStop(0.6,skyBot); grad.addColorStop(1,'#FF69B4');
    X.fillStyle=grad; X.fillRect(0,0,W,H);
    if(!nearBoss){{
        X.fillStyle='#FFD700'; X.beginPath(); X.arc(W-100,70,35,0,Math.PI*2); X.fill();
        X.fillStyle='#FFF8DC'; X.beginPath(); X.arc(W-102,68,30,0,Math.PI*2); X.fill();
        X.fillStyle='rgba(255,215,0,0.1)'; X.beginPath(); X.arc(W-100,70,60,0,Math.PI*2); X.fill();
    }}
    X.save(); X.translate(-cameraX*0.15,0);
    var treePos=[80,250,450,700,950,1200];
    for(var ti=0;ti<treePos.length;ti++){{ var tx=treePos[ti];
        X.fillStyle='#8B4513'; X.fillRect(tx-6,200,12,80);
        X.fillStyle='#228B22'; X.beginPath(); X.arc(tx,180,35,0,Math.PI*2); X.fill();
        X.fillStyle='#32CD32'; X.beginPath(); X.arc(tx-10,170,25,0,Math.PI*2); X.fill();
        X.fillStyle='#2E8B57'; X.beginPath(); X.arc(tx+12,175,22,0,Math.PI*2); X.fill();
    }}
    X.restore();
    X.save(); X.translate(-cameraX*0.3,0);
    var flowers=[120,200,320,410,530,620,740,850];
    for(var fi=0;fi<flowers.length;fi++){{ var fx=flowers[fi];
        X.fillStyle='#228B22'; X.fillRect(fx-1,280,2,15);
        var fc=['#FF69B4','#FF1493','#FFB6C0','#FF00FF','#DB7093','#FFA500','#FF6347','#FF4500'][fi%8];
        X.fillStyle=fc; X.beginPath(); X.arc(fx,275,5,0,Math.PI*2); X.fill();
        X.fillStyle='#FFD700'; X.beginPath(); X.arc(fx,275,2,0,Math.PI*2); X.fill();
    }}
    X.restore();
    if(nearBoss){{
        X.fillStyle='rgba(255,255,0,'+(0.03+Math.sin(Date.now()*0.01)*0.02)+')'; X.fillRect(0,0,W,H);
    }}
}}
...
function drawKidsBG(){{
    ...
    X.restore();
    X.save(); X.translate(-cameraX*0.4,0);
    for(var bi=0;bi<kidsBalls.length;bi++){{ var bl=kidsBalls[bi];
        if(bl.x<cameraX-100||bl.x>cameraX+W+100) continue;
        bl.x+=bl.vx;
        var groundY2=(TILE_ROWS-3)*TILE-bl.r;
        if(bl.y<groundY2){{ bl.y+=2; }} else {{ bl.y=groundY2; }}
        var colL=Math.floor(bl.x/TILE), colR=Math.floor((bl.x+bl.r*2)/TILE);
...
function drawLevel(){{
    ...
    else if(game.currentLevel===1) drawDesertBG();
    else if(game.currentLevel===5) drawGardenBG();
    else if(game.currentLevel===6) drawKidsBG();
    ...
    X.save(); X.translate(-cameraX,0);
    var rainbowColors=['#FF0000','#FF6600','#FFCC00','#00CC00','#0066FF','#CC00FF','#FF69B4','#00CED1'];
    for(var r=0;r<TILE_ROWS;r++) for(var c=Math.floor(cameraX/TILE);c<Math.ceil((cameraX+W)/TILE)+2;c++){{
        ...
        if(game.currentLevel===6){{
            var rcol=rainbowColors[(c+r)%rainbowColors.length];
            if(t===1){{ X.fillStyle=rcol; X.fillRect(tx,ty,TILE,TILE); X.strokeStyle='rgba(255,255,255,0.4)'; X.lineWidth=1; X.strokeRect(tx+0.5,ty+0.5,TILE-1,TILE-1); X.fillStyle='rgba(255,255,255,0.15)'; X.fillRect(tx,ty,TILE,3); }}
            else if(t===2){{ X.fillStyle=rainbowColors[(c+r+3)%rainbowColors.length]; X.fillRect(tx,ty,TILE,TILE); X.fillStyle='rgba(255,255,255,0.2)'; X.fillRect(tx,ty,TILE,3); X.strokeStyle='rgba(255,255,255,0.3)'; X.lineWidth=1; X.strokeRect(tx+0.5,ty+0.5,TILE-1,TILE-1); }}
        }} else {{
        ...
        if(t===5){{ X.fillStyle='#228B22'; X.fillRect(tx,ty,TILE,TILE); X.fillStyle='#32CD32'; X.fillRect(tx+2,ty,TILE-4,TILE/2); }}
    ...
    if(boss&&boss.alive){{
        ...
        if(game.currentLevel===0&&odairSprite.complete&&odairSprite.naturalWidth>0){{
            ...
            X.restore();
        }} else if(game.currentLevel===1&&pitoFaraImg.complete&&pitoFaraImg.naturalWidth>0){{
            X.save();
            X.translate(boss.x+boss.w/2,boss.y+boss.h/2);
            X.scale(boss.vx<0?1:-1,1);
            X.drawImage(pitoFaraImg,-boss.w/2,-boss.h/2,boss.w,boss.h);
            X.restore();
        }} else if(game.currentLevel===4&&thiaImg.complete&&thiaImg.naturalWidth>0){{
            X.save();
            X.translate(boss.x+boss.w/2,boss.y+boss.h/2);
            X.scale(boss.vx<0?1:-1,1);
            X.drawImage(thiaImg,-boss.w/2,-boss.h/2,boss.w,boss.h);
            X.restore();
        }} else if(game.currentLevel===5&&princesaDoxImg.complete&&princesaDoxImg.naturalWidth>0){{
            X.save();
            X.translate(boss.x+boss.w/2,boss.y+boss.h/2);
            X.scale(boss.vx<0?1:-1,1);
            X.drawImage(princesaDoxImg,-boss.w/2,-boss.h/2,boss.w,boss.h);
            X.restore();
    ...
    for(var i=0;i<lightningBolts.length;i++){{ var lb=lightningBolts[i];
        if(lb.flash>0){{ X.fillStyle='rgba(255,255,255,0.3)'; X.fillRect(0,0,W*3,H); }}
        X.strokeStyle='#FFFF00'; X.lineWidth=lb.w; X.shadowColor='#00FFFF'; X.shadowBlur=20;
        X.beginPath(); X.moveTo(lb.x,lb.y);
        for(var ly=lb.y;ly<lb.h;ly+=20){{ X.lineTo(lb.x+(Math.random()-0.5)*30,ly); }}
        X.stroke();
        X.strokeStyle='#FFFFFF'; X.lineWidth=2; X.shadowBlur=0;
        X.beginPath(); X.moveTo(lb.x,lb.y);
        for(var ly=lb.y;ly<lb.h;ly+=20){{ X.lineTo(lb.x+(Math.random()-0.5)*15,ly); }}
        X.stroke();
    }}
    for(var i=0;i<sneezeClouds.length;i++){{ var sc=sneezeClouds[i];
        X.fillStyle='rgba(255,255,255,'+(sc.life/60*0.5)+')';
        X.beginPath(); X.arc(sc.x,sc.y,sc.r,0,Math.PI*2); X.fill();
        X.fillStyle='rgba(200,200,255,'+(sc.life/60*0.3)+')';
        X.beginPath(); X.arc(sc.x+5,sc.y-3,sc.r*0.7,0,Math.PI*2); X.fill();
    }}
    ...
    for(var i=0;i<projectiles.length;i++){{ var p=projectiles[i]; X.fillStyle=p.color||(p.friendly?'#00ffcc':'#ff4444'); X.beginPath(); X.arc(p.x,p.y,p.w/2,0,Math.PI*2); X.fill(); }}
...
function drawLevelComplete(){{
    var isFinal=game.currentLevel===6;
    X.fillStyle=isFinal?'rgba(0,0,0,0.65)':'rgba(0,0,0,0.75)'; X.fillRect(0,0,W,H);
    X.textAlign='center';
    if(isFinal){{
        var rainbowH=['#FF0000','#FF6600','#FFCC00','#00CC00','#0066FF','#CC00FF','#FF69B4','#FF1493'];
        for(var ri=0;ri<8;ri++){{ X.fillStyle=rainbowH[ri]; X.globalAlpha=0.3+Math.sin(frameCount*0.04+ri)*0.2; X.fillRect(0,ri*H/8,W,H/8); }}
        X.globalAlpha=1;
        for(var ci=0;ci<60;ci++){{ var cx2=Math.random()*W, cy2=Math.random()*H, cs=4+Math.random()*8; X.fillStyle=rainbowH[ci%8]; X.globalAlpha=0.5+Math.random()*0.5; X.save(); X.translate(cx2,cy2); X.rotate(Math.random()*Math.PI*2); X.fillRect(-cs/2,-cs/4,cs,cs/2); X.restore(); }}
        X.globalAlpha=1;
        X.font='bold 52px Courier New'; X.fillStyle='#FFD700'; X.shadowColor='#FF6600'; X.shadowBlur=30; X.fillText('JOGO COMPLETO!',W/2,H/2-90); X.shadowBlur=0;
        X.font='bold 22px Courier New'; X.fillStyle='#FF69B4'; X.fillText('Xepa Grande foi derrotada!',W/2,H/2-40);
        X.font='16px Courier New'; X.fillStyle='#fff'; X.fillText('Todas as fases foram conquistas!',W/2,H/2+0);
        X.fillText('Score final: '+game.score+'  |  Sachês: '+game.coins,W/2,H/2+30);
    }} else {{
        for(var fi=0;fi<8;fi++){{ var fx=(W*0.15+fi*W*0.1)+Math.sin(frameCount*0.03+fi)*40; var fy=80+Math.cos(frameCount*0.04+fi*1.5)*30; var cols=['#ff00ff','#ffcc00','#00ffcc','#ff6600','#ffff00','#ff4488','#44ff88','#4488ff']; X.fillStyle=cols[fi]; X.shadowColor=cols[fi]; X.shadowBlur=15; X.beginPath(); for(var si=0;si<6;si++){{ var a=si*Math.PI/3+frameCount*0.05; var r1=8+Math.sin(frameCount*0.1+si)*3; var r2=16+Math.cos(frameCount*0.08+si)*5; if(si===0) X.moveTo(fx+Math.cos(a)*r2,fy+Math.sin(a)*r2); else X.lineTo(fx+Math.cos(a)*r2,fy+Math.sin(a)*r2); var a2=a+Math.PI/6; X.lineTo(fx+Math.cos(a2)*r1,fy+Math.sin(a2)*r1); }} X.closePath(); X.fill(); X.shadowBlur=0; }}
        X.font='bold 40px Courier New'; X.fillStyle='#ffcc00'; X.shadowColor='#ff6600'; X.shadowBlur=20; X.fillText('VITÓRIA!',W/2,H/2-70); X.shadowBlur=0;
        X.font='bold 20px Courier New'; X.fillStyle='#ff99cc'; X.fillText(game.defeatedBossName+' derrotado!',W/2,H/2-30);
        X.font='16px Courier New'; X.fillStyle='#fff'; X.fillText('Score: '+game.score,W/2,H/2+10); X.fillText('Sachês: '+game.coins,W/2,H/2+35);
    }}
    if(levelCompleteTimer<120){{
        var btnW=220, btnH=42, btnX=W/2-btnW/2, btnY=H/2+(isFinal?60:65);
        var hover=mouseX>=btnX&&mouseX<=btnX+btnW&&mouseY>=btnY&&mouseY<=btnY+btnH;
        X.fillStyle=hover?(isFinal?'#ff1493':'#00ccff'):(isFinal?'#cc0066':'#0088cc'); X.shadowColor=isFinal?'#ff1493':'#00ccff'; X.shadowBlur=hover?18:0;
        X.beginPath(); X.roundRect(btnX,btnY,btnW,btnH,8); X.fill(); X.shadowBlur=0;
        X.strokeStyle='#fff'; X.lineWidth=2; X.beginPath(); X.roundRect(btnX,btnY,btnW,btnH,8); X.stroke();
        X.font='bold 16px Courier New'; X.fillStyle='#fff'; X.fillText(isFinal?'VOLTAR AO MAPA':'CONTINUAR',W/2,btnY+28);
    }}
}}
...
document.addEventListener('keydown',function(e){{
    ...
    switch(game.state){{
        ...
        case 'playing':
            ...
            break;
        case 'levelcomplete': if((e.key==='Enter'||e.key===' ')&&levelCompleteTimer<120) game.state='worldmap'; break;
        case 'gameover':
...
C.addEventListener('click',function(e){{
    ...
    if(game.state==='levelcomplete'&&levelCompleteTimer<120){{
        var btnW=200, btnH=40, btnX=W/2-btnW/2, btnY=H/2+65;
        if(mouseX>=btnX&&mouseX<=btnX+btnW&&mouseY>=btnY&&mouseY<=btnY+btnH) game.state='worldmap';
    }}
}});

...
#loading {{ position:absolute; z-index:0; color:#ff99cc; text-align:center; }}
#loading .bar {{ width:300px; height:8px; background:#222; border-radius:4px; margin:10px auto; overflow:hidden; }}
#loading .bar div {{ height:100%; width:0%; background:linear-gradient(90deg,#ff99cc,#ffcc66); transition:width 0.3s; }}
</style>
</head>
<body>
<div id="loading"><div style="font-size:24px">GATAIADA</div><div style="font-size:12px;margin:6px 0">Carregando...</div><div class="bar"><div id="loadBar"></div></div><div id="loadStatus" style="font-size:10px;color:#888;margin-top:8px"></div></div>
<video id="intro" autoplay muted loop playsinline style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);min-width:100%;min-height:100%;width:auto;height:auto;z-index:1;object-fit:cover;"></video>
<canvas id="game" tabindex="0" style="position:relative;z-index:2;"></canvas>
<script>
...
introVideo.src = 'cats/abertura%20plataforma.mp4';
...
var catPortraitImgs = {{}};
var sheetsLoaded = 0;
var totalToLoad = CATS.length + 11;
var loadedCount = 0;
var loadBar = document.getElementById('loadBar');
var loadStatus = document.getElementById('loadStatus');
function updateLoadProgress(label) {{
    loadedCount++;
    if(loadBar) loadBar.style.width = Math.round(loadedCount/totalToLoad*100)+'%';
    if(loadStatus) loadStatus.textContent = label+' ('+loadedCount+'/'+totalToLoad+')';
...
CATS.forEach(function(cat, idx) {{
    ...
    img.onload = function() {{
        ...
        updateLoadProgress(cat.name);
    }};
    img.onerror = function() {{ updateLoadProgress(cat.name+' (erro)'); }};
    img.src = cat.sheet;
    var pImg = new Image();
    pImg.onload = function() {{ updateLoadProgress(cat.name+' retrato'); }};
    pImg.onerror = function() {{ updateLoadProgress(cat.name+' retrato (erro)'); }};
...
var extNames = ['odair sprite','odair face','casa','castelo','esfinge','matrix','saches','hotel','espaco','xepa boss','xepa enemy'];
extAssets.forEach(function(a,i) {{
    a.onload = function() {{ updateLoadProgress(extNames[i]); }};
    a.onerror = function() {{ updateLoadProgress(extNames[i]+' (erro)'); }};
}});
...
function playerDie(){{
    ...
    else{{ game.lives=game.maxHp; player.x=5*TILE; player.y=(TILE_ROWS-4)*TILE-player.h; player.vx=0; player.vy=0; player.hp=player.maxHp; player.invincible=90; if(game.grown) shrinkPlayer(); }}
}}
...
function drawLevel(){{
    ...
    if(cat.face){{
        ...
        X.drawImage(cat.face,4,4,28,28);
        X.restore();
    }}
...
function loop(t){{ var dt=t-lastTime; lastTime=t; update(dt); draw(); requestAnimationFrame(loop); }}

document.addEventListener('keydown',function(e){{
    keys[e.key]=true;
    switch(game.state){{
        case 'title': if(e.key==='Enter'){{ game.state='select'; selectIndex=0; introVideo.style.display='none'; }} break;
        case 'select':
        ...
        case 'worldmap':
            ...
            if(e.key==='Enter'){{ game.currentLevel=mapIndex; generateLevel(game.currentLevel); game.state='playing'; }}
        ...
        case 'playing':
            ...
            var num=parseInt(e.key); if(!isNaN(num)&&num>=0&&num<=6){{ game.currentLevel=num; generateLevel(game.currentLevel); }}
            break;
        case 'levelcomplete': if(e.key==='Enter'&&levelCompleteTimer<100) game.state='worldmap'; break;
        case 'gameover':
            if(e.key==='Enter'&&gameOverTimer<80){{ game.lives=7; game.chances=3; game.score=0; game.coins=0; game.grown=false; player.w=player.baseW; player.h=player.baseH; game.state='title'; introVideo.style.display=''; introVideo.play().catch(function(){{}}); }}
            break;
    }}
...
C.addEventListener('click',function(e){{
    ...
    if(game.state==='worldmap'&&mapHoverIndex>=0&&game.levelsUnlocked[mapHoverIndex]){{
        game.currentLevel=mapHoverIndex; generateLevel(game.currentLevel); game.state='playing';
    }}
}});
